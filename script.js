...
document.querySelector('.search-bar-container').addEventListener('submit', (event) => {
...
function searchPackages() {
  ...
  if (searchValue) {
    // Find all packages (box elements)
    const packages = document.querySelectorAll('.box');
    let found = false;

    // Remove the highlight class from all boxes (to reset highlighting)
    document.querySelectorAll('.box').forEach((box) => {
      box.classList.remove('highlight');
    });

    // Loop through each package and check if the search term matches the title
    packages.forEach((packageBox) => {
      const packageTitle = packageBox.querySelector('h3').textContent.toLowerCase();

      if (packageTitle.includes(searchValue)) {
        found = true;
        // Scroll into view if package matches
        packageBox.scrollIntoView({ behavior: 'smooth' });
        
        // Highlight the whole package box with animation
        packageBox.classList.add('highlight');
      }
    });

    // Optionally, show a message if no package matches
    if (!found) {
